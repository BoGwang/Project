...
public class MovieDaoImpl implements MovieDao {
	...
	public Movie select(String mno) throws CommonException {
		...
		try {
			movie = sqlSession.selectOne(MAPPER_NAMESPACE + ".select", mno);
	...
	public int movieCount() throws CommonException {
		...
		try {
			mcount = sqlSession.selectOne(MAPPER_NAMESPACE + ".count");
			
		} catch (Exception e) {
			logger.debug(e.getMessage());
			throw new CommonException("E10: 전체 영화 개수 가져오기 실패");
		}
	...
	public void delete(String mno) throws CommonException {		
		try {
			...
			logger.debug(e.getMessage());
			throw new CommonException("E88: 영화 삭제 실패");
		}
	}
